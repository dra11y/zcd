...
impl Interactive {
    fn show_interactive_menu(&self, _initial_completions: &[String], initial_partial: &str) -> Result<()> {
        let limit = self.limit;
        let filesystem_only = self.filesystem_only;

        let autocomplete_fn = move |input: &str| -> Result<Vec<String>, Box<dyn std::error::Error + Send + Sync>> {
            let current_dir = std::env::current_dir().ok();
